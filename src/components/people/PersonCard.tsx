...
/** Portrait layoutId used for the grid ↔ modal shared-element transition. */
export const portraitLayoutId = (slug: string) => `person-portrait-${slug}`;
    ...
    >
      ...
      <span className="block transition-transform duration-300 ease-precision group-hover:-translate-y-1.5">
        ...
        <span className="relative block">
          <motion.div
            layoutId={portraitLayoutId(person.slug)}
            className="relative aspect-square overflow-hidden [&_img]:transition-transform [&_img]:duration-500 group-hover:[&_img]:scale-[1.04]"
          >
            <PersonImage
              src={person.imageSrc}
              name={person.name}
        ...
        <span className="block p-5">
          <span className="block font-display text-lg font-semibold leading-snug tracking-[-0.01em] text-nsu-navy">
            {person.name}
          </span>
...
export function AlumniRow({ person, index, onOpen }: CardProps) {
  ...
  return (
    ...
    >
      <motion.span
        layoutId={portraitLayoutId(person.slug)}
        className="relative block h-14 w-14 shrink-0 overflow-hidden rounded-full"
      >
        <PersonImage
          src={person.imageSrc}
          name={person.name}
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate font-display text-[0.9375rem] font-semibold text-nsu-navy">
          {person.name}
        </span>
