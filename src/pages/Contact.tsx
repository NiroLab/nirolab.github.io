        ...
        <RevealItem key={title} y={32}>
          <div className="relative rounded-2xl border border-nsu-line bg-white p-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-mono text-lg font-semibold text-nsu-navy">
                {title}
              </h2>
            </div>
  ...
  return (
    ...
    >
      ...
      {[
        ...
        <circle key={i} cx={x} cy={y} r="4" fill={i === 3 ? "var(--nsu-gold)" : "var(--nsu-sky)"} />
      ))}
    </svg>
  );
}
          ...
          {pathways.map(({ icon: Icon, title, pitch, cta, href, external }) => (
            <RevealItem key={title} y={40}>
              <div className="group flex h-full flex-col rounded-2xl border border-nsu-line-dark bg-white/5 p-5 backdrop-blur-sm transition-colors duration-300 hover:border-nsu-sky">
                ...
                <h3 className="mt-6 font-mono text-[1.375rem] font-semibold tracking-[-0.01em] text-white">
                  {title}
                </h3>
                ...
                {external ? (
                  <a
                    href={href}
                    className="group/btn mt-7 inline-flex items-center gap-2 self-start rounded-full border border-nsu-sky/60 px-6 py-2.5 text-[0.9375rem] font-semibold text-nsu-skylight transition-colors hover:bg-nsu-sky/10 hover:text-white"
                  >
                    {cta}
