...
function HeroFactsCard({ project }: { project: Project }) {
  ...
  return (
    ...
    >
      {/* meta rail */}
      <dl className="space-y-3 border-b border-white/10 pb-5">
        ...
        <div className="border-b border-white/10 py-5">
          ...
          <ul className="space-y-2.5">
            {project.team.map((name) => {
              const person = matchedByName.get(name);
              return (
                <li key={name}>
                  {person ? (
                    <Link
                      to={`/people?m=${person.slug}`}
                      className="group flex items-center gap-3 rounded-lg p-1 -m-1 transition-colors hover:bg-white/5"
                    ...
                    >
                      ...
                      <span className="min-w-0">
                        <span className="block truncate text-sm font-medium text-white group-hover:text-nsu-sky">
                          {person.name}
                        </span>
                    ...
                    <span className="flex items-center gap-3 p-1 -m-1">
                      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-dashed border-white/20 font-mono text-[10px] text-slate-400">
                        {name
                          .split(/\s+/)
                          .filter(Boolean)
                          .slice(0, 2)
    ...
    >
      ...
      <div className="relative mx-auto w-full max-w-7xl px-5 pb-14 pt-28 md:px-8 md:pb-20">
        ...
        >
          <Link
            to="/projects"
            className="group inline-flex items-center gap-2 text-sm font-semibold text-nsu-sky transition-colors hover:text-white"
          >
          ...
          {project.team.map((name) => {
            ...
            const inner = (
              <>
                ...
                <span className="min-w-0">
                  <span className="block truncate font-display text-sm font-semibold text-nsu-navy">
                    {name}
                  </span>
            ...
            <RevealItem key={a.slug} y={0}>
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <h3 className="mt-1 font-display text-base font-semibold text-nsu-navy">
                    {a.title}
                  </h3>
          ...
          >
            ...
            >
              ...
              <span className="min-w-0">
                ...
                <span className="mt-1 block truncate font-display text-lg font-semibold text-white">
                  {prev.title}
                </span>
              </span>
    ...
    <section className="relative overflow-hidden bg-hero-gradient py-32 md:py-40">
      ...
      <div className="relative mx-auto max-w-3xl px-5 text-center md:px-8">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-4">
          <Link
            to="/projects"
            className="group inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-nsu-navy to-nsu-blue px-6 py-3 text-sm font-semibold text-white transition-transform active:scale-[0.97]"
          >
            <ArrowLeft className="h-4 w-4 transition-transform duration-300 group-hover:-translate-x-1" />
            All projects
          </Link>
